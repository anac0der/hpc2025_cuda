...
int main(int argc, char** argv)
{
    ...
    for (int k=1;k<=k_n;k++)
    {
        ...
        u0(i,j,k) = initial_cond(x,y,z);
        u1(i,j,k) = u0(i,j,k) + 
            (A_sq_tau/2.0) * laplace_7point_callable(initial_cond, x,y,z,hx,hy,hz);

        local_err0 = std::max(local_err0, fabs(exact_sol(x,y,z,0) - u0(i,j,k)));
        local_err1 = std::max(local_err1, fabs(exact_sol(x,y,z,tau) - u1(i,j,k)));
    }
    ...
    for (int n = 2; n <= K; n++)
    {
        ...
        for (int k = 1; k <= k_n; k++)
        {
            ...
            u(i, j, k) = A_sq_tau * laplace_7point(u1, i, j, k) +
                        2.0 * u1(i, j, k) - u0(i, j, k);

            double diff = fabs(exact_sol(x, y, z, tn) - u(i, j, k));
